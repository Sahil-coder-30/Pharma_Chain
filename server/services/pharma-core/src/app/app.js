...
// ── CSV Export Routes (requireAuth — manufacturer session JWT or service token) ─
//
// /core/export/:batchId         — streams the raw CSV file back (local dev fallback only)
// /core/export/:batchId/preview — paginated JSON preview for manufacturer dashboard UI
//
// In production: QR CSVs are downloaded via S3 pre-signed URLs (time-limited, secure).
// In development (no AWS creds): pharma-core saves to ./data/exports/{batchId}.csv
//   and returns http://localhost:4000/core/export/:batchId as the "s3DownloadUrl".
// IMPORTANT: /preview is registered BEFORE /:batchId so Express doesn't swallow it.
app.get('/core/export/:batchId/preview', requireAuth, exportPreviewController);
app.get('/core/export/:batchId',         requireAuth, localExportDownloadController);
